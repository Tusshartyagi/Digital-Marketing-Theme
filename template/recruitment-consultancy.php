...
/**
 * Template Name:  Recruitment Consultancy Page
 */
get_header();
?>
<!-- Slider Area Start -->
               ...
               $args = array(
                  ...
                  'tax_query' => array(
                     array(
                        ...
                        'field' => 'name',
                        'terms' => 'Recruitment Consultancy' // Term name
                     )
                  )
               );
...
<hr>
<section class="search-area pb-100 pt-100" id="testimonials">
   <div class="container px-lg-1 text-center">
      ...
      ?>
      <h2 class="pb-3 mx-auto w-lg-75" style="line-height: 1.3em;color:white;font-size:36px;text-align:center;z-index:999;position:relative"><?php echo esc_html($process_title); ?></h2>
      <p class="pb-5 mx-auto w-lg-50" style="line-height: 1.3em;color:white;font-size:18px;text-align:center;z-index:999;position:relative"><?php echo esc_html($business_description); ?></p>
   </div>
   <div class="container pt-4">
  <div class="row">
    <div class="col-lg-10 col-md-12 ml-auto">
      <div class="method owl-carousel my-sm-4" style="align-items:center">
        <?php
        // Query to fetch posts from Services custom post type
        $args = array(
          'post_type' => 'services',
          ...
          'tax_query' => array(
             array(
              'taxonomy' => 'services-box', // Taxonomy name
              'field' => 'name',
              'terms' => 'recruitment process' // Term name
            )
          )
        );
        $services_query = new WP_Query($args);
        ...
        if ($services_query->have_posts()) :
            ...
            while ($services_query->have_posts()) : $services_query->the_post();
               $counter++;
            ?>
            <div class="mx-auto" style="display:flex;align-items: flex-start;flex-direction: column;">
              <?php
              if (has_post_thumbnail()) {
                the_post_thumbnail('medium', ['class' => 'w-25 img-fluid mb-3']);
              }
              ?>
              <div class="single-service-content">
      ...
      <div class="pagination-info desktop-responsive" style="position: absolute; left: -150px;bottom: 3%;">
        <span class="current-page">1</span> / <span class="total-pages">8</span> Steps
      </div>
      <div class="pagination-info mobile-responsive" style="position: absolute;left: 38%;bottom: -8%;font-size: 18px;">
        <span class="current-page">1</span> / <span class="total-pages">8</span> Steps
      </div>
    </div>
  </div>
         ...
         $args = array(
            ...
            'tax_query' => array(
               array(
                  'taxonomy' => 'services-box', // Taxonomy name
                  'field' => 'name',
                  'terms' => 'Recruitment Choose Us', // Term name
               )
            )
         );
         $services_query = new WP_Query($args);
...
<section class="services-area pt-50 pb-50">
   ...
   <section class="services-area px-lg-2 px-0 py-2" id="services">
