...
/**
 * Template Name: Store Management Page
 */
get_header();
?>
               ...
               $args = array(
                  ...
                  'tax_query' => array(
                     array(
                        ...
                        'field' => 'name',
                        'terms' => 'Store Management' // Term name
                     )
                  )
               );
...
<section class="services-area pt-50 pb-50">
   ...
   <div class="container-fluid pb-0 px-lg-5 py-3">
      <div class="row">
         <div class="col-md-12">
            <div class="card-store store-slide owl-carousel">
               <?php
               // Query to fetch posts from Services custom post type
               $args = array(
                  'post_type' => 'sliders',
               ...
               if ($services_query->have_posts()) :
                  ...
                  while ($services_query->have_posts()) : $services_query->the_post();
                     $counter++;
               ?>
                     <div class="store-card m-lg-3" style="background-image:url('<?php if (has_post_thumbnail()) {   echo esc_url(get_the_post_thumbnail_url(get_the_ID(), 'medium'));   }   ?>')" style="width: 25%; max-width: 25%; overflow: hidden;">
                        <div class="overlay"></div>
                        <div class="border-0 text-left mb-2" style="position: absolute; bottom: 0; left: 0; right: 0;">
                           <div class="card-body choose-card">
                              <div class="content" style="color: white; position: relative; z-index: 1;font-size:20px">
                                 <h4 class="mb-1" style="color:white;font-size:42px"><?php the_title(); ?></h4>
                                 <div class="paragraph" style="font-size:20px"><?php the_content(); ?></div>
                              </div>
                           </div>
                        </div>
                     </div>
         ...
         $args = array(
            ...
            'tax_query' => array(
               array(
                  'taxonomy' => 'services-box', // Taxonomy name
                  'field' => 'name',
                  'terms' => 'Store Choose Us', // Term name
               )
            )
         );
         $services_query = new WP_Query($args);
